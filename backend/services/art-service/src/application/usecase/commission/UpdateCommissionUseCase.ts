...
import type { UpdateCommissionInput, UpdateCommissionResponse } from '../../../types/usecase-response';
...
export class UpdateCommissionUseCase implements IUpdateCommissionUseCase {
  ...
  async execute(id: string, userId: string, data: UpdateCommissionInput): Promise<UpdateCommissionResponse> {
    ...
    if (data.status === CommissionStatus.AGREED) {
      const negotiationStatuses: CommissionStatus[] = [
      ...
      if (!negotiationStatuses.includes(commission.status)) {
        throw new BadRequestError(
          'Cannot agree to a commission in current status',
        );
      }
    ...
    // 2. Deliver Artwork (Artist Only)
      ...
      if (!isArtist)
        throw new UnauthorizedError(COMMISSION_MESSAGES.USER_NOT_AUTHORIZED);
      if (
        commission.status !== CommissionStatus.LOCKED &&
        commission.status !== CommissionStatus.IN_PROGRESS
      ) {
        throw new BadRequestError(
          'Commission must be in progress or locked to deliver artwork',
        );
      }
      if (!data.finalArtwork)
        throw new BadRequestError(COMMISSION_MESSAGES.FINAL_ARTWORK_REQUIRED);
      ...
      const artworkObj = data.finalArtwork as { previewUrl?: string; originalUrl?: string; userId?: string } | null;
      if (artworkObj && (typeof data.finalArtwork === 'object')) {
        finalArt =
          artworkObj.previewUrl || artworkObj.originalUrl || '';
        finalImageUrl =
          artworkObj.originalUrl || artworkObj.previewUrl || '';

        // Fulfill requirement to match user IDs if present in upload metadata
        if (artworkObj.userId && artworkObj.userId !== userId) {
          throw new BadRequestError(
            'Artwork update failed: Uploading user does not match the commission artist',
          );
        }
      }
      ...
      const transferSuccess =
        await this._walletService.transferLockedCommissionFunds({
      ...
      if (!transferSuccess) {
        throw new BadRequestError(
          'Failed to transfer commission funds to your locked balance. Please try again.',
        );
      }

      historyDetails =
        'Artist delivered the final artwork. Funds moved to Artist locked balance.';
    }
    // 3. Complete & Distribute Funds (Requester Only)
      ...
      if (commission.status !== CommissionStatus.DELIVERED) {
        throw new BadRequestError(
          'Commission must be delivered before completion',
        );
      }
      ...
      const distributeSuccess =
        await this._walletService.distributeCommissionFunds({
      ...
      historyDetails =
        'Requester approved the delivery. Art Coins released to Artist.';
    }
    // 4. Handle Negotiation/Updates (Resets agreement)
      ...
      if (!negotiationStatuses.includes(commission.status)) {
        ...
        if (
          data.status !== CommissionStatus.DISPUTE_RAISED &&
          data.status !== CommissionStatus.CANCELLED &&
          data.status !== CommissionStatus.LOCKED
        ) {
          throw new BadRequestError(
            COMMISSION_MESSAGES.COMMISSION_NOT_IN_UPDATABLE_STATUS,
          );
        }
      }
      ...
      if (
        ...
        updatedData.referenceImages = data.referenceImages
          ? data.referenceImages.map(extractKey)
          : commission.referenceImages;
    ...
    const historyEntry = {
      action: String(updatedData.status || commission.status),
      userId: userId,
      timestamp: new Date(),
