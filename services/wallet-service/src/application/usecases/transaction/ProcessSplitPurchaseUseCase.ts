...
export class ProcessSplitPurchaseUseCase implements IProcessSplitPurchaseUseCase {
  ...
  async execute(dto: ProcessSplitPurchaseDTO): Promise<boolean> {
    logger.info(
      `Processing split purchase for art ${dto.artId}: Buyer=${dto.buyerId}, Seller=${dto.sellerId}, Admin=${dto.adminId}, Total=${dto.totalAmount}, Commission=${dto.commissionAmount}`
    );
