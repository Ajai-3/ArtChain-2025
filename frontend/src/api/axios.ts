...
apiClient.interceptors.request.use((config) => {
  ...
  const token = isAdminRequest ? state?.admin?.accessToken ?? null : state?.user?.accessToken ?? null;
...
const AUTH_ENDPOINTS = ["/api/v1/auth/login", "/api/v1/auth/register", "/api/v1/admin/login"];
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    const isAuthEndpoint = AUTH_ENDPOINTS.some((url) => originalRequest.url?.startsWith(url));

    if (error.response.status === 401 && !originalRequest._noRetry && !isAuthEndpoint) {
      if (!originalRequest._retry) originalRequest._retry = true;
        ...
        refreshRetryCount = 0;
        return Promise.reject({ status: 401, message: "Session expired. Please log in again.", fullError: error });
      }
      ...
      if (refreshRetryCount < MAX_REFRESH_RETRIES) {
        ...
        try {
          ...
          const response = await refreshClient.get<RefreshTokenResponse>("/api/v1/auth/refresh-token", { timeout: 30000 });
          ...
          const isAdmin = originalRequest.url?.includes("/api/v1/admin");
          store.dispatch(isAdmin ? adminLogout() : logout());
          return Promise.reject({ status: 401, message: "Session expired. Please log in again.", fullError: refreshError });
        }
      }
    }
