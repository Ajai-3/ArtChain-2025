...
const RecentCommissionsTable: React.FC<RecentCommissionsTableProps> = ({
  commissions,
}) => {
        ...
        <Table>
          ...
          <TableBody>
            {commissions.map((comm, index) => {
              const style =
                statusStyles[comm.status.toUpperCase()] ??
                getFallback(comm.status);
              return (
                <TableRow key={comm.id}>
                    ...
                    <div className='flex gap-2 py-1'>
                      ...
                      <div className='flex items-center gap-2'>
                        <div className='h-6 w-6 rounded-full overflow-hidden bg-zinc-200 flex-shrink-0'>
                          <img
                            src={
                              comm.artistProfileImage ||
                              `https://ui-avatars.com/api/?name=${comm.artistName}&background=random`
                            }
                            alt=''
                      ...
                      <div className='pl-8 flex flex-col gap-1'>
                        <div className='flex items-center gap-1.5'>
                          <div className='h-6 w-6 rounded-full overflow-hidden bg-zinc-200 flex-shrink-0'>
                            <img
                              src={
                                comm.clientProfileImage ||
                                `https://ui-avatars.com/api/?name=${comm.clientName}&background=random`
                              }
                              alt=''
                              className='h-full w-full object-cover'
                            />
                  ...
                  <TableCell>
                    <span className='font-mono text-emerald-500 font-bold text-sm'>
                      {comm.amount > 0 ? `${comm.amount} AC` : '-'}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span
                      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-semibold tracking-wide h-6 ${style.bg} ${style.border} ${style.text}`}
                    >
                      {style.label}
                    </span>
                  </TableCell>
                    ...
                    {comm.status.toUpperCase() === 'COMPLETED' && (
                      <Button
                        ...
                        disabled={
                          ...
                          downloadMutation.variables?.id === comm.id
                        }
