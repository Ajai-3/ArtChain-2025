...
const CommissionStatsChart: React.FC<CommissionStatsChartProps> = ({
  stats,
  filter,
  onFilterChange,
}) => {
  ...
  const displayData = hasData
    ...
    : [{ value: 1, name: 'No Active Pipeline', fill: '#f4f4f5' }];
        ...
        <div className='flex justify-between'>
          ...
          >
            <TabsList className='h-7 w-auto bg-muted/50 p-1'>
              <TabsTrigger
                value='today'
                className='text-[10px] px-2 h-7 data-[state=active]:bg-background'
              >
                1D
              </TabsTrigger>
              <TabsTrigger
          ...
          <ResponsiveContainer width='100%' height='100%'>
            <FunnelChart>
              <Tooltip
                ...
                cursor={false}
              />
              <Funnel dataKey='value' data={displayData} isAnimationActive>
                <LabelList
                  position='right'
                  fill='#888'
                  stroke='none'
