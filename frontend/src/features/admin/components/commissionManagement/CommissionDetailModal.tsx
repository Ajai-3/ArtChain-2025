...
const CommissionDetailModal: React.FC<CommissionDetailModalProps> = ({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='sm:max-w-[850px] bg-zinc-950 border-zinc-800 text-zinc-100 p-0 scrollbar overflow-hidden'>
          ...
          <div className='flex items-center justify-between'>
            ...
            <div className='flex flex-col items-end'>
              <span className='text-2xl font-bold flex items-center gap-1 text-emerald-400'>
                {commission.budget}{' '}
                <Coins className='w-5 h-5 fill-emerald-400/20' />
              </span>
        ...
        <div className='max-h-[60vh] overflow-y-auto p-6 custom-scrollbar'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
            ...
            <div className='space-y-6'>
                ...
                commission.referenceImages.length > 0 && (
                  <div className='space-y-2'>
                    ...
                    <div className='grid grid-cols-3 gap-2'>
                      {commission.referenceImages.map(
              ...
              {(commission.finalArtwork || commission.finalImageUrl) && (
                <div className='space-y-2'>
                  <h4 className='text-sm font-semibold text-zinc-400 uppercase tracking-wide'>
                    Final Artwork
                  </h4>
                  <div className='rounded-lg overflow-hidden border border-zinc-800 relative group aspect-video bg-zinc-900'>
                    <img
                      src={commission.finalArtwork || commission.finalImageUrl}
                      alt='Final Delivery'
                ...
                <div className='flex items-center gap-3 bg-zinc-900/30 p-3 rounded-xl border border-zinc-800/50'>
                  <div className='w-10 h-10 rounded-full bg-zinc-800 overflow-hidden flex-shrink-0'>
                    {commission.requester?.profileImage ? (
                      <img
                        src={commission.requester.profileImage}
                ...
                <div className='flex items-center gap-3 bg-indigo-500/5 p-3 rounded-xl border border-indigo-500/20'>
                  <div className='w-10 h-10 rounded-full bg-zinc-800 overflow-hidden flex-shrink-0 border border-indigo-500/30'>
                    {commission.artist?.profileImage ? (
                      <img
                        src={commission.artist.profileImage}
                        alt={commission.artist.username}
                        className='w-full h-full object-cover'
                      />
                    ) : (
                      <div className='w-full h-full flex items-center justify-center text-indigo-500 text-xs'>
                        ?
                      </div>
                    )}
                  </div>
                  <div className='flex-1 min-w-0'>
                    <span className='text-xs text-indigo-400/70 uppercase tracking-wider block'>
                      Artist
                    </span>
              ...
              {commission.status === 'DISPUTE_RAISED' && (
                <div className='bg-red-500/10 border border-red-500/20 rounded-xl p-4 space-y-3'>
                  ...
                  <div className='bg-red-500/5 p-3 rounded-lg border border-red-500/10'>
                    <p className='text-xs text-red-300/70 uppercase tracking-wider mb-1 font-bold'>
                      Report Reason
                    </p>
                    <p className='text-sm text-zinc-300 leading-relaxed italic'>
                      "
                      {commission.disputeReason ||
                        'No specific reason provided by requester.'}
                      "
                    </p>
                  </div>
                  <div className='pt-2 flex gap-2 w-full'>
                    <Button
                      ...
                      onClick={() =>
                        onResolveDispute?.(commission.id, 'REFUND')
                      }
                    ...
                    <Button
                      ...
                      onClick={() =>
                        onResolveDispute?.(commission.id, 'RELEASE')
                      }
                      disabled={isResolving}
                    >
                      Release to Artist
              ...
              {commission.history && commission.history.length > 0 && (
                <div className='space-y-3 pt-2 border-t border-zinc-900'>
                  ...
                  <div className='space-y-3 pl-1'>
                    {[...commission.history].reverse().map((event, i) => (
                      <div
                        key={i}
