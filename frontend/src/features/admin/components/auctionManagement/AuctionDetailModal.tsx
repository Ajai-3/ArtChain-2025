...
const AuctionDetailModal: React.FC<AuctionDetailModalProps> = ({
  ...
  const { data: auction, isLoading: isLoadingAuction } =
  ...
  const allBids = useMemo(
    () => bidsData?.pages.flatMap((page) => page.bids) || [],
    [bidsData],
  );
            ...
            <div className="w-full md:w-[40%] bg-zinc-50 dark:bg-zinc-900/30 p-6 flex flex-col border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-800">
                      ...
                      className={`px-2.5 py-1 rounded-full text-[10px] font-bold border shadow-sm ${
                        auction.status === 'ACTIVE'
                          ...
                          : auction.status === 'SCHEDULED'
                            ? 'bg-blue-100 text-indigo-700 border-blue-200 dark:bg-indigo-900/50 dark:text-indigo-400 dark:border-indigo-800'
                            : auction.status === 'ENDED' && auction.winner
                              ? 'bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/50 dark:text-yellow-500 dark:border-yellow-800'
                              : auction.status === 'UNSOLD'
                                ? 'bg-red-100 text-gray-700 border-red-200 dark:bg-gray-900/50 dark:text-gray-400 dark:border-gray-800'
              ...
              <div className="mb-4 bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                ...
                <div className="flex items-end gap-1">
                  ...
                  <span className="text-sm font-medium text-zinc-500 mb-1.5">
                    AC
                  </span>
                </div>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                      {auction.host?.username || 'Unknown'}
                    </span>
                  </div>
                </div>
                {auction.host?.username && (
                  <Link
                    to={`/${auction.host.username}`}
                    target="_blank"
                  ...
                  <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-900/10 dark:to-orange-900/10 border border-yellow-100 dark:border-yellow-900/30 rounded-xl">
                    <div className="relative flex-shrink-0">
                      {auction.winner.profileImage ? (
                        <img
                          src={auction.winner.profileImage}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-yellow-700 dark:text-yellow-500 font-bold uppercase tracking-wider mb-0.5">
                        Auction Winner
                      </p>
                      ...
                      <p className="text-sm font-medium text-zinc-500">
                        ...
                        <span className="text-green-600 dark:text-green-500">
                          {auction.currentBid || auction.startPrice} AC
                        </span>
                      </p>
                    </div>
                    ...
                    {auction.winner?.username && (
                      <Link
                        to={`/${auction.winner.username}`}
                        target="_blank"
                        className="p-2 text-yellow-600/50 hover:text-yellow-700 hover:bg-yellow-100/50 rounded-full transition-colors ml-2"
                      >
                        <ExternalLink className="w-4 h-4" />
                      </Link>
                    )}
                  ...
                  {isLoadingBids ? (
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10">
                        <tr>
                          <th className="px-6 py-2.5 text-left font-medium text-zinc-500 text-[10px] uppercase tracking-wider">
                            Bidder
                          </th>
                          <th className="px-6 py-2.5 text-right font-medium text-zinc-500 text-[10px] uppercase tracking-wider">
                            Amount
                          </th>
                          <th className="px-6 py-2.5 text-right font-medium text-zinc-500 text-[10px] uppercase tracking-wider">
                            Time
                              ...
                              <div className="flex items-center gap-3">
                                {bid.bidder?.profileImage ? (
                                  <img
                                    ...
                                    className="w-8 h-8 rounded-full object-cover border border-zinc-100 dark:border-zinc-800"
                                  />
                                ) : (
                                  <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
                                    <User className="w-4 h-4 text-zinc-400" />
