...
export const ReportTargetDisplay: React.FC<ReportTargetDisplayProps> = ({
  ...
  if (isError || !data)
    return (
      <span className='text-destructive text-xs'>Content unavailable</span>
    );
    ...
    return (
      <div className='flex items-center justify-between gap-4'>
        ...
        <Button
          ...
          size='sm'
          onClick={() =>
            window.open(`/${user?.username}/art/${art?.artName}`, '_blank')
          }
          className='shrink-0'
        >
          <ExternalLink className='h-4 w-4 mr-2' />
        ...
        <div className='flex items-center justify-between'>
          ...
          <Button
            ...
            onClick={() => {
              if (art && art.user) {
                window.open(
                  `/${art.user.username}/art/${art.artName}`,
                  '_blank',
                );
              } else {
                console.warn(
                  'Art or Art Owner not found for navigation context',
                );
              }
            }}
