...
export const AuctionInfoPanel = ({
  ...
  return (
    <div className='flex flex-col gap-6'>
      ...
      <div className='space-y-4'>
        ...
        <div className='flex flex-wrap items-center gap-4 text-sm text-muted-foreground'>
          ...
          <div className='flex items-center gap-1.5 bg-muted/50 px-2.5 py-1 rounded-md border border-border/50'>
            ...
            <span>
              {auction.status === 'ACTIVE' ? 'Ends' : 'Started'}{' '}
              {new Date(
      ...
      {/* Artist Card */}
      <div
        className='flex items-center gap-4 p-4 bg-card border border-border rounded-xl shadow-sm hover:border-primary/20 transition-colors cursor-pointer'
        onClick={() => navigate(`/profile/${auction.hostId}`)}
      >
        <Avatar className='h-12 w-12 border-2 border-background ring-2 ring-border/50'>
          <AvatarImage src={auction.host?.profileImage} />
          <AvatarFallback>
            {auction.host?.username?.substring(0, 2).toUpperCase()}
          </AvatarFallback>
        </Avatar>
        ...
        <Button variant='secondary' size='sm' className='font-semibold h-8'>
          View Profile
        </Button>
      </div>
