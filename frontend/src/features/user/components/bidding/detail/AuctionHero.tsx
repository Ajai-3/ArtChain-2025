...
export const AuctionHero = ({
  ...
  return (
    <div
      className={`relative flex-1 rounded-2xl border-2 bg-card overflow-hidden group shadow-sm min-h-[400px] flex items-center justify-center ${isEnded ? 'border-border' : 'border-border'}`}
    >
      <img
        src={
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none">
        ...
        {isEnded && (
          <Badge
            variant={isUnsold ? 'destructive' : 'secondary'}
            className={`backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-2 text-xs font-bold shadow-lg border-0 ${isUnsold ? 'bg-neutral-600/90 text-white hover:bg-neutral-700' : 'bg-amber-500/90 text-white hover:bg-amber-600'}`}
          >
            {isUnsold ? (
      ...
      {isEnded && !isUnsold && auction.winner && (
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/95 via-black/80 to-transparent p-6 pt-20 flex flex-col items-center justify-end text-center animate-in slide-in-from-bottom-5 pointer-events-none">
          <p className="text-amber-300 font-bold tracking-widest uppercase text-xs mb-3 drop-shadow-md">
            🎉 Winning Bidder
          </p>
          ...
          <div className="flex items-center gap-2 text-white/90 font-medium bg-white/10 px-3 py-1 rounded-full backdrop-blur-md border border-white/10">
            ...
            <span className="text-emerald-400 font-bold text-lg">
              {auction.currentBid.toLocaleString()} AC
            </span>
          </div>
        </div>
      )}
      ...
      {isEnded && isUnsold && (
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-6 pt-20 flex flex-col items-center justify-end text-center animate-in slide-in-from-bottom-5 pointer-events-none">
          ...
          <p className="text-white/60 text-sm max-w-xs">
            Reserve price was not met or no bids were placed during the auction
            period.
          </p>
        </div>
      )}
