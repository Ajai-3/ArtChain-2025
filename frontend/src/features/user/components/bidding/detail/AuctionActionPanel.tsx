...
export const AuctionActionPanel = ({
  ...
  return (
    <div
      className={`rounded-2xl border-2 p-5 flex flex-col gap-6 shadow-xl relative overflow-hidden bg-card ${isLive ? 'border-emerald-500/40 shadow-emerald-500/10' : isEnded ? (isUnsold ? 'border-neutral-500/30' : 'border-amber-500/40 shadow-amber-500/10') : 'border-indigo-500/40 shadow-indigo-500/10'}`}
    >
      {/* Decorative BG Blob */}
      <div
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
        {/* Price Card */}
        <div
          className={`p-4 rounded-xl border backdrop-blur-sm flex flex-col justify-center ${bgColor} ${borderColor}`}
        >
          <span
            className={`text-[10px] uppercase font-bold tracking-wider mb-1 block opacity-80 ${
              isLive
        ...
        <div className="p-4 bg-muted/30 rounded-xl border border-border/60 backdrop-blur-sm flex flex-col justify-center">
          {isEnded ? (
            <div className="flex flex-col">
              ...
              <span
                className={`font-bold flex items-center gap-2 ${isUnsold ? 'text-neutral-500' : 'text-amber-600 dark:text-amber-400'}`}
              >
                {isUnsold ? <>Auction Closed (Unsold)</> : <>Sold to Winner</>}
              </span>
