...
export const DetailStatsCard = ({
  ...
  return (
      ...
      className={`
            ...
            ${
              isLive
                ...
                : isEnded
                  ? isUnsold
                    ? 'bg-card border-neutral-500 shadow-neutral-500/10'
                    : 'bg-card border-red-500/50 shadow-red-500/10'
    ...
    >
      ...
      <div
        className={`absolute -top-20 -right-20 w-40 h-40 rounded-full blur-[50px] opacity-30 pointer-events-none 
      ...
      <div className="min-h-0 flex-1 flex flex-col gap-2">
        <div className="flex justify-between items-start gap-4 shrink-0">
          <h1
            className="text-2xl md:text-3xl font-extrabold leading-tight tracking-tight line-clamp-2"
            title={auction.title}
          >
            {auction.title}
          </h1>
          ...
          {isEnded && (
            <div
              className={`shrink-0 p-2 rounded-lg ${isUnsold ? 'bg-neutral-500/10 text-neutral-500' : 'bg-red-500/10 text-red-500'}`}
            >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <span
            className={`text-[10px] uppercase font-bold tracking-wider mb-1 block ${
              isLive
                ? 'text-emerald-500'
                : isEnded
                  ? isUnsold
        ...
        <div className="px-4 py-2 bg-muted/30 rounded-xl border border-border/50 backdrop-blur-sm">
          {isEnded ? (
            <div className="h-full flex flex-col justify-center">
              ...
              <span
                className={`font-bold flex items-center gap-1.5 ${isUnsold ? 'text-neutral-600' : 'text-red-600 dark:text-red-400'}`}
              >
    ...
    `}
        onClick={onPlaceBid}
        disabled={!isLive || isCurrentUserIsHost}
      >
        {isLive ? (
          isCurrentUserIsHost ? (
            <span className="flex items-center gap-2 text-white">
              <Clock className="h-5 w-5" /> You are the Host
            </span>
          ) : (
