    ...
    <CardContent className={`p-4 pt-0 space-y-2 flex-1`}>
      {isLive && (
        <div className="p-2 bg-emerald-500/10 rounded-xl border border-emerald-500/20 shadow-sm">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-bold text-emerald-600/80 dark:text-emerald-500/80 self-end mb-1.5">
              AC
            </span>
          </div>
        </div>
      ...
      {isScheduled && (
        <div className="p-3 bg-indigo-500/10 rounded-xl border border-indigo-500/20 shadow-sm">
          <span className="text-[10px] text-indigo-700 dark:text-indigo-400 uppercase font-bold tracking-wider mb-0.5 block">
            Starting Price
          </span>
          <div className="flex items-center gap-1.5">
            <span className="text-2xl font-black tabular-nums text-indigo-700 dark:text-indigo-400">
              {formatNumber(auction.startPrice)}
            </span>
            <span className="text-xs font-bold text-indigo-600/80 dark:text-indigo-500/80 self-end mb-1.5">
              AC
            </span>
          </div>
        </div>
      ...
      {isEnded && (
        <div
          className={`p-3 rounded-xl border shadow-sm grayscale-[0.2] ${isUnsold ? 'bg-neutral-500/5 border-neutral-500/20' : 'bg-amber-500/5 border-amber-500/20'}`}
        >
          <span
            className={`text-[10px] uppercase font-bold tracking-wider mb-0.5 block ${isUnsold ? 'text-neutral-500' : 'text-amber-700 dark:text-amber-500'}`}
          >
            {isUnsold ? 'Starting Price (Unsold)' : 'Final Sold Price'}
          </span>
          <div className="flex items-center gap-1.5">
            <span
              className={`text-2xl font-black tabular-nums ${isUnsold ? 'text-neutral-500' : 'text-amber-700 dark:text-amber-500'}`}
            >
              {formatNumber(
                auction.currentBid > 0
                  ? auction.currentBid
            ...
            <span
              className={`text-xs font-bold self-end mb-1.5 ${isUnsold ? 'text-neutral-400' : 'text-amber-600/60'}`}
            >
              AC
            </span>
          </div>
        ...
        <div className="space-y-1.5">
          <div className="flex justify-between items-center text-[10px] uppercase tracking-wider font-bold text-muted-foreground">
            ...
            <span className="text-[10px] normal-case opacity-90 font-semibold bg-muted px-1.5 py-0.5 rounded border border-border">
              {new Date(
                isLive ? auction.endTime : auction.startTime,
