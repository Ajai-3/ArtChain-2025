...
const ChatArea: React.FC<ChatAreaProps> = ({
  ...
  const { messages, loadMoreMessages, hasMore, isLoading, isFetchingMore } =
  ...
  const handleVideoCall = useCallback(() => {
    if (selectedConversation) {
      const pId =
        selectedConversation.memberIds?.find((id) => id !== currentUserId) ||
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChatInput
          onSendMessage={(text) =>
            onSendMessage({ content: text, mediaType: 'TEXT' })
          }
      ...
      <Dialog
        open={!!previewImage}
        onOpenChange={(open) => !open && !isUploading && setPreviewImage(null)}
      >
        <DialogContent>
          <DialogHeader>
